...
- if you changed code while addressing review feedback, rerun the earlier validation from the bug-or-feature lane and any nearby integration or end-to-end tests when feasible before continuing
...
- if you changed code while fixing CI-related problems, rerun the earlier validation from the bug-or-feature lane and any nearby integration or end-to-end tests when feasible before checking CI again
...
- `post_ready_for_landing_comment`: 10 minutes
- `post_needs_judgment_comment`: 10 minutes
- The bug/feature validation lanes are ACP steps. They choose and run their own validation commands instead of relying on hardcoded runtime helpers.

ACP steps without an explicit timeout in the workflow currently rely on the `acp-bridge` flow runtime default. At the moment that default is 15 minutes, so if a step such as `extract_intent`, `judge_solution`, `bug_or_feature`, `judge_refactor`, `comment_and_close_pr`, `comment_and_escalate_ready_for_landing`, or `comment_and_escalate_needs_judgment` should have a different budget, that must be stated explicitly in the workflow file.
