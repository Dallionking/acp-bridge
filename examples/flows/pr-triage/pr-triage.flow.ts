...
import { defineFlow, extractJsonObject } from "acp-bridge/flows";
import { selectLocalCodexReviewText } from "./review-text.js";

const FLOW_DIR = ".acp-bridge-flow";
const MAIN_SESSION = {
  handle: "main",
};
...
async function prepareWorkspace(pr) {
  ...
  const workdir = await fs.mkdtemp(path.join(os.tmpdir(), `acp-bridge-pr${pr.prNumber}-`));
