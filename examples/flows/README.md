...
```bash
acp-bridge flow run examples/flows/echo.flow.ts \
...
acp-bridge flow run examples/flows/shell.flow.ts \
  --input-json '{"text":"hello from shell"}'

acp-bridge flow run examples/flows/workdir.flow.ts

acp-bridge flow run examples/flows/two-turn.flow.ts \
...
These examples are examples only. They do not define `acp-bridge` core product
behavior.
