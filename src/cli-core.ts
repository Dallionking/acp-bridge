...
import { getAcpBridgeVersion } from "./version.js";
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (argv.includes("--version") || argv.includes("-V")) {
    process.stdout.write(`${getAcpBridgeVersion()}\n`);
    return;
  }
  ...
  if (argv[2] === "__queue-owner") {
    try {
      ...
      process.stderr.write(`[acp-bridge] queue owner failed: ${message}\n`);
