...
function parseAcpBridgeState(raw: unknown): SessionAcpBridgeState | undefined {
  const record = asRecord(raw);
  if (!record) {
    return undefined;
  }

  const state: SessionAcpBridgeState = {};
  ...
  if (Array.isArray(record.config_options)) {
    state.config_options = record.config_options as SessionAcpBridgeState["config_options"];
  }

  const sessionOptions = asRecord(record.session_options);
  if (sessionOptions) {
    const parsedSessionOptions: NonNullable<SessionAcpBridgeState["session_options"]> = {};
...
export function parseSessionRecord(raw: unknown): SessionRecord | null {
  ...
  if (
    typeof record.acp-bridge_record_id !== "string" ||
  ...
  const eventLog = parseEventLog(record.event_log, record.acp-bridge_record_id);
  ...
  return {
    ...
    acp-bridgeRecordId: record.acp-bridge_record_id,
    ...
    acp-bridge: parseAcpBridgeState(record.acp-bridge),
  };
}
