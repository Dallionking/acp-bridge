...
function sessionFilePath(acp-bridgeRecordId: string): string {
  const safeId = encodeURIComponent(acp-bridgeRecordId);
  return path.join(sessionBaseDir(), `${safeId}.json`);
}

function sessionBaseDir(): string {
  return path.join(os.homedir(), ".acp-bridge", "sessions");
}
...
export async function writeSessionRecord(record: SessionRecord): Promise<void> {
  await measurePerf("session.write_record", async () => {
    ...
    const file = sessionFilePath(record.acp-bridgeRecordId);
...
export async function resolveSessionRecord(sessionId: string): Promise<SessionRecord> {
  ...
  const exactEntries = entries.filter(
    (entry) => entry.acp-bridgeRecordId === sessionId || entry.acpSessionId === sessionId,
  );
  const exactRecords = (
  ...
  const suffixEntries = entries.filter(
    (entry) => entry.acp-bridgeRecordId.endsWith(sessionId) || entry.acpSessionId.endsWith(sessionId),
  );
  const suffixRecords = (
...
export async function pruneSessions(options: PruneOptions = {}): Promise<PruneResult> {
  ...
  for (const record of records) {
    const safeId = encodeURIComponent(record.acp-bridgeRecordId);
