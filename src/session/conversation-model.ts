...
export function cloneSessionAcpBridgeState(
  state: SessionAcpBridgeState | undefined,
): SessionAcpBridgeState | undefined {
  if (!state) {
    return undefined;
  }
...
export function recordSessionUpdate(
  ...
  const acp-bridge = ensureAcpBridgeState(state);
  ...
  switch (update.sessionUpdate) {
    ...
    case "available_commands_update": {
      acp-bridge.available_commands = update.availableCommands
        .map((entry) => entry.name)
        .filter((entry) => typeof entry === "string" && entry.trim().length > 0);
      break;
    }
    case "current_mode_update": {
      acp-bridge.current_mode_id = update.currentModeId;
      break;
    }
    case "config_option_update": {
      acp-bridge.config_options = deepClone(update.configOptions);
      break;
    }
    default:
      break;
