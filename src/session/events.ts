...
export class SessionEventWriter {
  ...
  static async open(
    ...
    const lock = await acquireEventsLock(record.acp-bridgeRecordId);
    ...
    const activePath = activeEventPath(record.acp-bridgeRecordId);
    ...
    const segmentCount =
      Number.isInteger(record.eventLog.segment_count) && record.eventLog.segment_count > 0
        ...
        : (await countExistingSegments(record.acp-bridgeRecordId, maxSegments)) || 1;
    ...
    await measurePerf("session.events.append_batch", async () => {
      for (const message of messages) {
        ...
        if (this.activeSizeBytes > 0 && this.activeSizeBytes + lineBytes > this.maxSegmentBytes) {
          await rotateSegments(this.record.acp-bridgeRecordId, this.maxSegments);
          this.activePath = activeEventPath(this.record.acp-bridgeRecordId);
