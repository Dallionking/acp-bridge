...
import { decodeAcpBridgeRuntimeHandleState, writeHandleState } from "./runtime/public/handle-state.js";
import { normalizeRuntimeDetails, probeRuntime } from "./runtime/public/probe.js";
import { deriveAgentFromSessionKey, type AcpBridgeHandleState } from "./runtime/public/shared.js";
...
export const ACP_BRIDGE_BACKEND_ID = "acp-bridge";

const ACP_BRIDGE_CAPABILITIES: AcpRuntimeCapabilities = {
  controls: ["session/set_mode", "session/set_config_option", "session/status"],
};

type AcpBridgeRuntimeLike = AcpRuntime & {
...
export class AcpBridgeRuntime implements AcpBridgeRuntimeLike {
  ...
  getCapabilities(_input?: { handle?: AcpRuntimeHandle }): AcpRuntimeCapabilities {
    return ACP_BRIDGE_CAPABILITIES;
  }
  ...
  private resolveManagerHandle(handle: AcpRuntimeHandle): {
    handle: AcpRuntimeHandle;
    state: AcpBridgeHandleState;
  } {
    const state = this.resolveHandleState(handle);
    return {
      handle: {
        ...handle,
        acp-bridgeRecordId: state.acp-bridgeRecordId ?? handle.acp-bridgeRecordId ?? handle.sessionKey,
      },
      state,
    };
  }

  private resolveHandleState(handle: AcpRuntimeHandle): AcpBridgeHandleState {
    const decoded = decodeAcpBridgeRuntimeHandleState(handle.runtimeSessionName);
    if (decoded) {
      return {
        ...decoded,
        acp-bridgeRecordId: decoded.acp-bridgeRecordId ?? handle.acp-bridgeRecordId,
...
export function createAcpRuntime(options: AcpRuntimeOptions): AcpBridgeRuntime {
  return new AcpBridgeRuntime(options);
}
