  ...
  try {
    ...
    if (params.verbose) {
      process.stderr.write(
        `[acp-bridge] replayed desired mode ${params.desiredModeId} on fresh ACP session ${params.sessionId} (previous ${params.previousSessionId})\n`,
      );
    }
  } catch (error) {
...
async function replayDesiredModel(params: {
  ...
  try {
    ...
    if (params.verbose) {
      process.stderr.write(
        `[acp-bridge] replayed desired model ${params.desiredModelId} on fresh ACP session ${params.sessionId} (previous ${params.previousSessionId})\n`,
      );
    }
  } catch (error) {
...
async function replayDesiredConfigOptions(params: {
  ...
  for (const [configId, value] of Object.entries(params.desiredConfigOptions)) {
    try {
      ...
      if (params.verbose) {
        process.stderr.write(
          `[acp-bridge] replayed desired config option ${configId} on fresh ACP session ${params.sessionId} (previous ${params.previousSessionId})\n`,
        );
      }
    } catch (error) {
...
export async function connectAndLoadSession(
  ...
  const desiredModeId = getDesiredModeId(record.acp-bridge);
  const desiredModelId = getDesiredModelId(record.acp-bridge);
  const desiredConfigOptions = getDesiredConfigOptions(record.acp-bridge);
  ...
  if (options.verbose) {
    if (storedProcessAlive) {
      process.stderr.write(
        `[acp-bridge] saved session pid ${record.pid} is running; reconnecting with loadSession\n`,
      );
    } else if (shouldReconnect) {
      process.stderr.write(
        `[acp-bridge] saved session pid ${record.pid} is dead; respawning agent and attempting session/load\n`,
      );
    }
  }
  ...
  if (createdFreshSession) {
    try {
      ...
      if (options.verbose) {
        process.stderr.write(`[acp-bridge] ${formatErrorMessage(error)}\n`);
      }
      throw error;
    }
