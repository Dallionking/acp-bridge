...
function applyConfigOptionsToRecord(
  ...
  const acp-bridgeState = cloneSessionAcpBridgeState(record.acp-bridge) ?? {};
  acp-bridgeState.config_options = structuredClone(configOptions);
  record.acp-bridge = acp-bridgeState;
}
...
function createInitialRecord(params: {
  ...
  return {
    ...
    acp-bridgeRecordId: params.recordId,
...
function statusSummary(record: SessionRecord): string {
  const parts = [
    `session=${record.acp-bridgeRecordId}`,
...
export class AcpRuntimeManager {
  ...
  private async readPendingPersistentClient(
    ...
    const pendingClient = this.pendingPersistentClients.get(record.acp-bridgeRecordId);
    if (!pendingClient) {
      return undefined;
    }
    if (!pendingClient.hasReusableSession(record.acpSessionId)) {
      this.pendingPersistentClients.delete(record.acp-bridgeRecordId);
  ...
  private async refreshClosedState(record: SessionRecord): Promise<boolean> {
    if (!this.closingActiveRecords.has(record.acp-bridgeRecordId)) {
      return record.closed === true;
    }
    const latest = await this.options.sessionStore.load(record.acp-bridgeRecordId).catch(() => undefined);
    record.closed = true;
    record.closedAt = latest?.closedAt ?? record.closedAt ?? isoNow();
    if (latest?.acp-bridge) {
      record.acp-bridge = {
        ...record.acp-bridge,
        ...latest.acp-bridge,
      };
  ...
  private async retainPersistentClientAfterTurn(input: {
    ...
    const isPersistentRecord = !record.acp-bridgeRecordId.includes(":oneshot:");
    if (!isPersistentRecord || record.closed || !client.hasReusableSession(record.acpSessionId)) {
      return false;
    }
    const previousClient = this.pendingPersistentClients.get(record.acp-bridgeRecordId);
    this.pendingPersistentClients.set(record.acp-bridgeRecordId, client);
  ...
  async ensureSession(input: {
    ...
    if (
      ...
      this.closingActiveRecords.delete(existing.acp-bridgeRecordId);
    ...
    try {
      ...
      this.closingActiveRecords.delete(record.acp-bridgeRecordId);
  ...
  startTurn(input: {
    ...
    void (async () => {
      ...
      let acp-bridgeState: ReturnType<typeof cloneSessionAcpBridgeState>;
      let client: AcpClient | null = null;
      try {
        record = await this.requireRecord(input.handle.acp-bridgeRecordId ?? input.handle.sessionKey);
        conversation = cloneSessionConversation(record);
        acp-bridgeState = cloneSessionAcpBridgeState(record.acp-bridge);
        ...
        activeController = {
          ...
          setSessionMode: async (modeId: string) => {
            ...
            const nextState = cloneSessionAcpBridgeState(acp-bridgeState) ?? {};
            nextState.desired_mode_id = modeId;
            acp-bridgeState = nextState;
          },
        ...
        this.activeControllers.set(runtimeRecord.acp-bridgeRecordId, activeController);
        runtimeClient.setEventHandlers({
          onSessionUpdate: (notification) => {
            acp-bridgeState = recordSessionUpdate(runtimeConversation, acp-bridgeState, notification);
          ...
          onClientOperation: (operation: ClientOperation) => {
            acp-bridgeState = recordClientOperation(runtimeConversation, acp-bridgeState, operation);
            trimConversationForRuntime(runtimeConversation);
            emitParsed({
        ...
        const { sessionId, resumed, loadError } = pendingClient
          ...
          : await connectAndLoadSession({
              ...
              onClientAvailable: (controller) => {
                ...
                this.activeControllers.set(runtimeRecord.acp-bridgeRecordId, controller);
              },
        ...
        runtimeRecord.acp-bridge = acp-bridgeState;
        ...
        if (record) {
          this.activeControllers.delete(record.acp-bridgeRecordId);
          this.closingActiveRecords.delete(record.acp-bridgeRecordId);
        }
        queue.close();
      }
  ...
  async getStatus(handle: AcpRuntimeHandle): Promise<AcpRuntimeStatus> {
    const record = await this.requireRecord(handle.acp-bridgeRecordId ?? handle.sessionKey);
    return {
      ...
      details: {
        ...
        ...(record.acp-bridge?.config_options !== undefined
          ? { configOptions: structuredClone(record.acp-bridge.config_options) }
          : {}),
      },
    };
  }
  ...
  async setMode(
    ...
    const record = await this.requireRecord(handle.acp-bridgeRecordId ?? handle.sessionKey);
    const controller = this.activeControllers.get(record.acp-bridgeRecordId);
  ...
  async cancel(handle: AcpRuntimeHandle): Promise<void> {
    const controller = this.activeControllers.get(handle.acp-bridgeRecordId ?? handle.sessionKey);
  ...
  async close(
    ...
    const record = await this.requireRecord(handle.acp-bridgeRecordId ?? handle.sessionKey);
    if (this.activeControllers.has(record.acp-bridgeRecordId)) {
      this.closingActiveRecords.add(record.acp-bridgeRecordId);
    }
    ...
    if (options.discardPersistentState) {
      ...
      await this.closePendingPersistentClient(record.acp-bridgeRecordId);
    }
    record.closed = true;
    ...
    try {
      ...
      if (!client.supportsCloseSession()) {
        throw new AcpRuntimeError(
          ...
          `Agent does not support session/close for ${record.acp-bridgeRecordId}.`,
        );
      }
