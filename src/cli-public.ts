...
type ConfigurePublicCliOptions = {
  ...
  listBuiltInAgents: (agents: ResolvedAcpBridgeConfig["agents"]) => string[];
  detectAgentToken: (argv: string[]) => AgentTokenScan;
  registerAgentCommand: (program: Command, agentName: string, config: ResolvedAcpBridgeConfig) => void;
  registerDefaultCommands: (program: Command, config: ResolvedAcpBridgeConfig) => void;
