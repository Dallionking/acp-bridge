...
export class FlowRunner {
    ...
    return await this.runWithHeartbeat(
      ...
      async () => {
        ...
        await cancelSessionPrompt({
          sessionId: boundSession.acp-bridgeRecordId,
        });
      },
    );
  }
  ...
  private async ensureSessionBinding(
    ...
    const binding: FlowSessionBinding = {
      ...
      agentCommand: agent.agentCommand,
      cwd: agent.cwd,
      acp-bridgeRecordId: created.record.acp-bridgeRecordId,
  ...
  private async refreshSessionBinding(binding: FlowSessionBinding): Promise<FlowSessionBinding> {
    const record = await resolveSessionRecord(binding.acp-bridgeRecordId);
    return {
      ...binding,
  ...
  private async runPersistentPrompt(
    ...
    const beforeRecord = await resolveSessionRecord(binding.acp-bridgeRecordId);
    ...
    try {
      await sendSessionDirect({
        sessionId: binding.acp-bridgeRecordId,
        prompt,
      ...
      const afterRecord = await resolveSessionRecord(sessionInfo.acp-bridgeRecordId);
  ...
  private async runIsolatedPrompt(
    ...
    let acp-bridgeState: SessionRecord["acp-bridge"] | undefined;
    ...
    const syntheticRecord = createSyntheticSessionRecord({
      ...
      conversation,
      acp-bridgeState: cloneSessionAcpBridgeState(acp-bridgeState),
