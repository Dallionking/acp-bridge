...
export function resolveGeminiAcpStartupTimeoutMs(): number {
  const raw = process.env.ACP_BRIDGE_GEMINI_ACP_STARTUP_TIMEOUT_MS;
...
export function resolveClaudeAcpSessionCreateTimeoutMs(): number {
  const raw = process.env.ACP_BRIDGE_CLAUDE_ACP_SESSION_CREATE_TIMEOUT_MS;
  if (typeof raw === "string" && raw.trim().length > 0) {
    const parsed = Number(raw);
    if (Number.isFinite(parsed) && parsed > 0) {
      return Math.round(parsed);
...
export function buildClaudeAcpSessionCreateTimeoutMessage(): string {
  return [
    ...
    "In harnessed or non-interactive runs, prefer --approve-all with nonInteractivePermissions=deny, upgrade Claude Code and the Claude ACP adapter, or use acp-bridge claude exec as a one-shot fallback.",
  ].join(" ");
}
...
async function buildCopilotAcpUnsupportedMessage(command: string): Promise<string> {
  const parts = [
    ...
    "acp-bridge copilot expects a Copilot CLI release that supports --acp --stdio.",
  ];
