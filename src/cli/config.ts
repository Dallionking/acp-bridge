...
export type ResolvedAcpBridgeConfig = {
  defaultAgent: string;
...
function defaultGlobalConfigPath(): string {
  return path.join(os.homedir(), ".acp-bridge", "config.json");
}

function projectConfigPath(cwd: string): string {
  return path.join(path.resolve(cwd), ".acp-bridgerc.json");
}
...
export async function loadResolvedConfig(cwd: string): Promise<ResolvedAcpBridgeConfig> {
...
export function toConfigDisplay(config: ResolvedAcpBridgeConfig): {
  defaultAgent: string;
  defaultPermissions: PermissionMode;
  nonInteractivePermissions: NonInteractivePermissionPolicy;
  authPolicy: AuthPolicy;
