...
async function findRoutedSessionOrThrow(
  ...
  const createCmd = sessionName
    ? `acp-bridge ${agentName} sessions new --name ${sessionName}`
    : `acp-bridge ${agentName} sessions new`;
  throw new NoSessionError(
    `⚠ No acp-bridge session found (searched up to ${walkBoundary}).\nCreate one: ${createCmd}`,
  );
}
...
export async function handlePrompt(
  ...
  if (globalFlags.verbose && result.loadError) {
    process.stderr.write(`[acp-bridge] loadSession failed, started fresh session: ${result.loadError}\n`);
  }
}
      ...
      process.stdout.write(
        `${JSON.stringify({
          ...
          error: {
            ...
            data: { acp-bridgeCode: "EXEC_DISABLED" },
          },
        })}\n`,
      );
    } else {
...
function printCancelResultByFormat(
  ...
  if (
    emitJsonResult(format, {
      ...
      acp-bridgeRecordId: result.sessionId || "unknown",
...
function printSetModeResultByFormat(
  ...
  if (
    emitJsonResult(format, {
      ...
      acp-bridgeRecordId: result.record.acp-bridgeRecordId,
      acp-bridgeSessionId: result.record.acpSessionId,
...
export async function handleSessionsClose(
  ...
  const closed = await closeSession(record.acp-bridgeRecordId);
...
export async function handleSessionsNew(
  ...
  if (replaced) {
    await closeSession(replaced.acp-bridgeRecordId);
    if (globalFlags.verbose) {
      process.stderr.write(`[acp-bridge] soft-closed prior session: ${replaced.acp-bridgeRecordId}\n`);
    }
  }
  ...
  if (globalFlags.verbose) {
    ...
    process.stderr.write(`[acp-bridge] created ${scope}: ${created.acp-bridgeRecordId}\n`);
  }
...
function printSessionDetailsByFormat(record: SessionRecord, format: OutputFormat): void {
  ...
  process.stdout.write(`id: ${record.acp-bridgeRecordId}\n`);
...
function printSessionHistoryByFormat(
  ...
  process.stdout.write(
    `session: ${record.acp-bridgeRecordId} (${visible.length}/${history.length} shown)\n`,
  );
  if (visible.length === 0) {
