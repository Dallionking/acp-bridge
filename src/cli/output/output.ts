...
function renderAuthRequiredHint(params: RenderableOutputError): string {
  ...
  if (methodIds.length === 0) {
    return "hint: run `acp-bridge config show` to locate the active config, then add the required credential under `auth` and retry.";
  }

  const configKeys = methodIds.map((methodId) => `\`auth.${methodId}\``);
  return `hint: run \`acp-bridge config show\` to locate the active config, then add ${formatDisjunction(configKeys)} and retry.`;
}
...
export function getTextErrorRemediationHints(params: RenderableOutputError): string[] {
  ...
  if (params.code === "NO_SESSION") {
    ...
    return [
      "hint: the saved ACP session is missing or stale; start a fresh session with `acp-bridge <agent> sessions new`, then retry.",
    ];
  }

  if (lowerMessage.includes("does not support session/load")) {
    return [
      "hint: this adapter cannot resume saved ACP sessions; create a fresh one with `acp-bridge <agent> sessions new` instead of reusing `--resume-session`.",
    ];
  }
  ...
  if (
    ...
    return [
      ...
      "hint: if you do not need the old backend session, start a fresh one with `acp-bridge <agent> sessions new` and retry.",
    ];
  }
...
class QuietOutputFormatter implements OutputFormatter {
  ...
  private formatUsageLine(usage: Record<string, unknown> | undefined): string | undefined {
    ...
    return parts.length > 0 ? `[acp-bridge] tokens: ${parts.join(" ")}` : undefined;
  }

  private formatCostLine(cost: unknown): string | undefined {
    if (typeof cost === "number" && Number.isFinite(cost)) {
      return `[acp-bridge] cost: ${formatMetadataNumber(cost)}`;
    }

    if (typeof cost === "string" && cost.trim()) {
      return `[acp-bridge] cost: ${cost.trim()}`;
    }
    ...
    const currency =
      typeof record.currency === "string" && record.currency.trim()
        ...
        : "";
    return `[acp-bridge] cost: ${formatMetadataNumber(amount)}${currency}`;
  }
}
