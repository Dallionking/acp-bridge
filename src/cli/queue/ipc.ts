...
async function maybeRecoverStaleOwnerAfterProtocolMismatch(params: {
  ...
  if (params.verbose) {
    process.stderr.write(
      `[acp-bridge] dropped stale queue owner metadata after protocol mismatch for session ${params.sessionId} (${detailCode})\n`,
    );
  }
...
export async function trySubmitToRunningOwner(
  ...
  if (submitted) {
    if (options.verbose) {
      process.stderr.write(
        `[acp-bridge] queued prompt on active owner pid ${owner.pid} for session ${options.sessionId}\n`,
      );
    }
    return submitted;
  }
...
export async function tryCancelOnRunningOwner(options: {
  ...
  if (cancelled !== undefined) {
    if (options.verbose) {
      process.stderr.write(
        `[acp-bridge] requested cancel on active owner pid ${owner.pid} for session ${options.sessionId}\n`,
      );
    }
    return cancelled;
  }
...
export async function trySetModeOnRunningOwner(
  ...
  if (submitted) {
    if (verbose) {
      process.stderr.write(
        `[acp-bridge] requested session/set_mode on owner pid ${owner.pid} for session ${sessionId}\n`,
      );
    }
    return true;
  }
...
export async function trySetModelOnRunningOwner(
  ...
  if (submitted) {
    if (verbose) {
      process.stderr.write(
        `[acp-bridge] requested session/set_model on owner pid ${owner.pid} for session ${sessionId}\n`,
      );
    }
    return true;
  }
...
export async function trySetConfigOptionOnRunningOwner(
  ...
  if (response) {
    if (verbose) {
      process.stderr.write(
        `[acp-bridge] requested session/set_config_option on owner pid ${owner.pid} for session ${sessionId}\n`,
      );
    }
    return response;
  }
