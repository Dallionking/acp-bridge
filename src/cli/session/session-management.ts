...
function persistSessionOptions(
  ...
  const next =
    ...
    } satisfies NonNullable<NonNullable<SessionRecord["acp-bridge"]>["session_options"]>);
  ...
  if (!record.acp-bridge) {
    return;
  }

  delete record.acp-bridge.session_options;
}
  ...
  if (existing) {
    ...
    if (requestedModel) {
      const result = await setSessionModel({
        sessionId: existing.acp-bridgeRecordId,
