...
export function resolveQueueOwnerSpawnArgs(argv: readonly string[] = process.argv): string[] {
  const override = process.env.ACP_BRIDGE_QUEUE_OWNER_ARGS;
  if (override) {
    ...
    throw new Error("acp-bridge self-spawn failed: invalid ACP_BRIDGE_QUEUE_OWNER_ARGS");
  }

  const entry = argv[1];
  if (!entry || entry.trim().length === 0) {
    throw new Error("acp-bridge self-spawn failed: missing CLI entry path");
  }
...
export function buildQueueOwnerSpawnOptions(payload: string): {
  ...
  return {
    ...
    env: {
      ...process.env,
      ACP_BRIDGE_QUEUE_OWNER_PAYLOAD: payload,
    },
    windowsHide: true,
  };
}
