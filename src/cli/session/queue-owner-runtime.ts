...
export async function runSessionQueueOwner(options: QueueOwnerRuntimeOptions): Promise<void> {
    ...
    void applyPendingCancel().catch((error) => {
      if (options.verbose) {
        process.stderr.write(
          `[acp-bridge] failed to apply deferred cancel: ${formatErrorMessage(error)}\n`,
        );
      }
    });
  };
  ...
  try {
    ...
    if (options.verbose) {
      process.stderr.write(
        `[acp-bridge] queue owner ready for session ${options.sessionId} (ttlMs=${ttlMs}, maxQueueDepth=${maxQueueDepth})\n`,
      );
    }
    ...
    if (options.verbose) {
      process.stderr.write(`[acp-bridge] queue owner stopped for session ${options.sessionId}\n`);
    }
  }
}
