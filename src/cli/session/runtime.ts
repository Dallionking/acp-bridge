...
async function applyPromptModelIfAdvertised(params: {
  ...
  const availableModels = params.record.acp-bridge?.available_models;
  assertRequestedModelSupported({
    requestedModel,
    models: Array.isArray(availableModels)
      ? {
          currentModelId: params.record.acp-bridge?.current_model_id ?? "",
  ...
  if (params.record.acp-bridge?.current_model_id === requestedModel) {
...
function emitPromptRetryNotice(params: {
  ...
  process.stderr.write(
    `[acp-bridge] prompt failed (${formatErrorMessage(params.error)}), retrying in ${params.delayMs}ms ` +
...
async function runSessionPrompt(options: RunSessionPromptOptions): Promise<SessionSendResult> {
  ...
  let acp-bridgeState = cloneSessionAcpBridgeState(record.acp-bridge);
  ...
  client.setEventHandlers({
    ...
    onSessionUpdate: (notification) => {
      ...
      acp-bridgeState = recordConversationSessionUpdate(conversation, acp-bridgeState, notification);
    ...
    onClientOperation: (operation) => {
      ...
      acp-bridgeState = recordConversationClientOperation(conversation, acp-bridgeState, operation);
      trimConversationForRuntime(conversation);
  ...
  try {
    return await withInterrupt(
      async () => {
        ...
        if (options.verbose) {
          process.stderr.write(
            `[acp-bridge] ${formatPerfMetric("prompt.connect_and_load", Date.now() - connectStartedAt)}\n`,
          );
        }
        ...
        for (let attempt = 0; ; attempt++) {
          try {
                  ...
                  attempt === 0 && options.onPromptActive
                    ? async () => {
                        try {
                          ...
                          if (options.verbose) {
                            process.stderr.write(
                              "[acp-bridge] onPromptActive hook failed: " +
            ...
            if (options.verbose) {
              process.stderr.write(
                `[acp-bridge] ${formatPerfMetric("prompt.agent_turn", Date.now() - promptStartedAt)}\n`,
              );
            }
            break;
          } catch (error) {
            ...
            if (lastExit?.unexpectedDuringPrompt && options.verbose) {
              process.stderr.write(
                "[acp-bridge] agent disconnected during prompt (" +
                  lastExit.reason +
                  ", exit=" +
                  lastExit.exitCode +
                  ", signal=" +
        ...
        return {
          ...toPromptResult(response.stopReason, record.acp-bridgeRecordId, client),
          record,
          resumed,
          loadError,
        };
    ...
    if (options.verbose) {
      process.stderr.write(`[acp-bridge] ${formatPerfMetric("prompt.total", stopTotalTimer())}\n`);
    } else {
      stopTotalTimer();
    }
