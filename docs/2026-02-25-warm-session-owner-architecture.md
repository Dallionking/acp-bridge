...
`acp-bridge` currently keeps prompt queue ownership in the foreground caller process. That enables queue reuse, but it also means a direct CLI call can appear "hung" after `[done] end_turn` while the owner waits for idle TTL expiry.
...
Today, `acp-bridge` prompt flow behaves like this:
...
- `acp-bridge <agent> prompt ...` exits immediately after turn completion.
...
Split `acp-bridge` into two roles:
...
### Session record (`~/.acp-bridge/sessions/*.json`)
...
### Owner lease record (`~/.acp-bridge/queues/*.lock`)
...
1. `acp-bridge <agent> sessions ensure`
2. `time acp-bridge <agent> prompt "say hi"`
3. verify immediate exit after done
4. send second prompt; verify warm reuse
5. `acp-bridge <agent> sessions close`; verify owner stops and lease removed
