---
title: ACP_BRIDGE Error Strategy
author: Onur <2453968+osolmaz@users.noreply.github.com>
date: 2026-02-22
---

# ACP_BRIDGE Error Strategy

Permanent machine-facing error contract for `acp-bridge` orchestrators.

## Scope

This document defines how `acp-bridge` should represent errors across:
...
- Provide stable `acp-bridge` codes for orchestrator logic.
...
`acp-bridge` should expose both:

- `acp-bridge` machine codes (stable, small enum for orchestration)
...
- `code`: required stable `acp-bridge` code.
