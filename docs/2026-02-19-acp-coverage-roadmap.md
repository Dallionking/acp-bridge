...
What acp-bridge implements from the ACP spec today and what is not yet implemented.

## Supported Now

| ACP Method                   | acp-bridge Feature                                     | Supported |
| ---------------------------- | ------------------------------------------------ | --------- |
...
| `session/cancel`             | Graceful cancel + `acp-bridge <agent> cancel`          | yes       |
| `session/request_permission` | `--approve-all`, `--approve-reads`, `--deny-all` | yes       |
| `session/set_mode`           | `acp-bridge <agent> set-mode <mode>`                   | yes       |
| `session/set_config_option`  | `acp-bridge <agent> set <key> <value>`                 | yes       |
...
- `acp-bridge <agent> cancel` sends `session/cancel` through the queue path and keeps the session alive for follow-up prompts.
...
- acp-bridge handles `fs/read_text_file` and `fs/write_text_file` as client-authority operations.
...
- acp-bridge handles full terminal lifecycle: create, output, wait_for_exit, kill, release.
- The implementation includes process tracking, output buffering, and cleanup behavior for terminal IDs.

#### Authentication

- acp-bridge handles `authenticate` when adapters request it.
...
- `session/list`: would expose adapter-side session inventory in addition to acp-bridge local store listing.
...
Things acp-bridge needs that aren't in the ACP spec:

- [ ] **Permission policies** — Path-based rules (`allow reads to src/`, `deny
writes to .env`). Beyond the current all-or-nothing modes.
- [ ] **Multi-agent orchestration** — Agent A prompts Agent B through acp-bridge.
      Session bridging.
