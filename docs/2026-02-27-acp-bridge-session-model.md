...
- `acp-bridge_record_id`: acp-bridge local record id (stable storage id).
...
- `acp-bridge_record_id` is always required in local storage.
...
For each `acp-bridge_record_id`:

```text
~/.acp-bridge/sessions/<acp-bridge_record_id>.stream.ndjson
~/.acp-bridge/sessions/<acp-bridge_record_id>.stream.1.ndjson
~/.acp-bridge/sessions/<acp-bridge_record_id>.stream.2.ndjson
...
~/.acp-bridge/sessions/<acp-bridge_record_id>.json
~/.acp-bridge/sessions/<acp-bridge_record_id>.stream.lock
```

Rules:

- `*.stream*.ndjson` is authoritative history.
- `<acp-bridge_record_id>.json` is the local checkpoint/index.
...
- no acp-bridge-only control/event wrappers in the stream,
...
## Session Checkpoint Schema (`acp-bridge.session.v1`)

`session.json` is derived from replay + local runtime state, with top-level conversation and top-level acp-bridge state.

```json
{
  "schema": "acp-bridge.session.v1",
  "acp-bridge_record_id": "019c....",
  "acp_session_id": "019c....",
  ...
  "event_log": {
    "active_path": "/home/user/.acp-bridge/sessions/019c....stream.ndjson",
    "segment_count": 3,
...
1. Acquire `<acp-bridge_record_id>.stream.lock`.
...
- no acp-bridge envelope/event schema accepted on ACP stream,
- checkpoint validator for `acp-bridge.session.v1`,
- contract tests that assert `--format json --json-strict` emits ACP-only lines.

## Non-Goals

- custom stream schema layered over ACP,
- mixed ACP + acp-bridge envelope events in one stream,
