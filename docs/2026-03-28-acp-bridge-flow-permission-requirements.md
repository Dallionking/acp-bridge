---
title: acp-bridge Flow Permission Requirements
...
# acp-bridge Flow Permission Requirements

## Status

This document specifies the intended flow permission model.

It is a design target for `acp-bridge` flows and should not be read as if every part
is already enforced in the current runtime.

## Why this exists

Some flows are meaningfully more powerful than ordinary `acp-bridge` prompts.
...
- do not make `approve-all` the global default for all `acp-bridge` usage
...
Rerun with: acp-bridge flow run examples/flows/pr-triage/pr-triage.flow.ts --approve-all
...
- ordinary `acp-bridge` usage may stay conservative by default
