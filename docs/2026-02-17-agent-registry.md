---
title: acp-bridge Agent Registry
...
When you run `acp-bridge <agent> ...`:

1. agent token is normalized (trim + lowercase)
2. if it matches a built-in key, `acp-bridge` uses the mapped command
3. if it does not match, `acp-bridge` treats it as a raw command
...
```bash
acp-bridge --agent ./my-custom-acp-server 'summarize this repo'
```
