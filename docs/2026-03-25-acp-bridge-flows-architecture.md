---
title: acp-bridge Flows Architecture
description: Execution model, runtime boundary, and design principles for acp-bridge flows.
author: acp-bridge contributors
date: 2026-03-25
---

# acp-bridge Flows Architecture

## Why this document exists

`acp-bridge` flows add a small workflow layer on top of the existing ACP runtime.
...
This document describes the shape that `acp-bridge` flows use:
...
- conversations stay in the existing `~/.acp-bridge/sessions/*.json` store

## Core position

`acp-bridge` should stay a small ACP client with composable primitives.
...
- Make multi-step ACP workflows first-class in `acp-bridge`
...
Flows are normal `.ts` modules that import helpers from `acp-bridge/flows` and
...
```ts
import { defineFlow, acp, action, compute, checkpoint } from "acp-bridge/flows";
...
`acp-bridge` should not move toward a fully functional whole-flow API where user code
...
- user code imports helpers from `acp-bridge/flows`
...
- changing how the loader resolves `acp-bridge/flows`
...
`cwd` already exists in `acp-bridge` session handling.
...
[`docs/2026-03-28-acp-bridge-flow-permission-requirements.md`](2026-03-28-acp-bridge-flow-permission-requirements.md).
...
Conversation state stays in the existing `acp-bridge` session store:

- `~/.acp-bridge/sessions/*.json`

Flow state lives separately under:

- `~/.acp-bridge/flows/runs/`
...
- [`2026-03-26-acp-bridge-flow-trace-replay.md`](2026-03-26-acp-bridge-flow-trace-replay.md)
...
`acp-bridge` supports a forgiving default because models sometimes wrap JSON with
extra text.
...
- Keep workload-specific logic in user flow files or example files, not in
  `acp-bridge` core product behavior
...
```bash
acp-bridge flow run <file> [--input-json <json> | --input-file <path>]
```

Run state is persisted under `~/.acp-bridge/flows/runs/`.
...
Core flow support in `acp-bridge` should stay generic:
...
- normal authoring imports from `acp-bridge/flows`
