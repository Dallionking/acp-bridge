---
title: acp-bridge Built-in Agent Launch Ownership
description: Ownership and launch model for built-in ACP agent adapters.
author: acp-bridge contributors
date: 2026-04-06
---

# acp-bridge Built-in Agent Launch Ownership

## Why this document exists

`acp-bridge` currently knows about built-in agents such as `codex` and `claude`, but
the actual launch behavior is not owned cleanly in one place.

That split becomes fragile when `acp-bridge` is embedded inside another long-running
...
`acp-bridge` should fully own the built-in agent launcher contract.

That means `acp-bridge` should be the single source of truth for:
...
built-in launcher defaults for agents that `acp-bridge` already defines.

## What this means in practice

When a caller asks for a built-in agent such as `claude` or `codex`, `acp-bridge`
should resolve that request through built-in metadata owned inside `acp-bridge`.
...
If the adapter package is already installed locally, `acp-bridge` should resolve its
...
If the adapter package is not installed locally, `acp-bridge` still needs a way to
...
default just because an application embeds `acp-bridge`.
...
If a local adapter is already present, `acp-bridge` should use it. If it is not
present, `acp-bridge` should decide how to fetch, install, or prepare it through its
own built-in path.

## What embedding applications should do

Embedding applications should pass the built-in agent name and let `acp-bridge`
...
`codex`, or other built-in agents that `acp-bridge` already owns.

In other words:

- user override: embedding app may pass through
- built-in default: owned by `acp-bridge`

## Error handling requirement

When an ACP child exits before initialize completes, `acp-bridge` should fail fast
...
- No requirement that every built-in adapter be installed as a normal
  dependency of the `acp-bridge` package.
...
- built-in agent ownership lives in `acp-bridge`
- built-in adapter pins live in `acp-bridge`
...
That keeps `acp-bridge` small while still making built-in agent execution reliable in
