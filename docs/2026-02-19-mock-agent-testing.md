...
`acp-bridge` now includes a local ACP mock agent and integration tests that exercise the
CLI against real JSON-RPC traffic over stdio.

## What It Is

- `test/mock-agent.ts`: a standalone ACP agent implementation used only for tests.
- `test/integration.test.ts`: end-to-end tests that run `acp-bridge` as a subprocess with
...
1. `acp-bridge` starts the mock agent process via `--agent`.
2. `acp-bridge` sends ACP requests (`initialize`, `session/new`, `session/prompt`).
3. Mock agent sends ACP client-method requests back to `acp-bridge`:
   - `fs/read_text_file`
   - `fs/write_text_file`
   - `terminal/create`
   - `terminal/output`
   ...
   - `terminal/kill`
   - `terminal/release`
4. Mock agent emits `session/update` chunks so `acp-bridge` output formatters can render
...
4. Add a new case to `test/integration.test.ts` that calls `acp-bridge ... exec "<prompt>"`.
5. Assert:
   - process exit code
