...
`acp-bridge` is a CLI client that speaks ACP over stdio.
...
- If load fails with not-found style errors, `acp-bridge` falls back to `newSession`.
- Prompt responses and notifications are streamed through the active formatter.

## Session persistence

Session metadata is stored in `~/.acp-bridge/sessions/*.json`.
...
This lets `acp-bridge` resume conversational context by default.
...
- one `acp-bridge` process becomes queue owner for the active turn
...
`acp-bridge` tracks permission stats (requested/approved/denied/cancelled) and uses them for exit-code behavior.
