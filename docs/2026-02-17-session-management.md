---
title: acp-bridge Session Management
description: How acp-bridge resumes, names, stores, and closes sessions including pid tracking and subprocess lifecycle.
...
`acp-bridge` is conversational by default.
...
- create default session: `acp-bridge codex sessions new`
- create named session: `acp-bridge codex sessions new --name backend`
- default prompt: `acp-bridge codex 'fix tests'`
- named prompt: `acp-bridge codex -s backend 'fix API'`
...
Stored under `~/.acp-bridge/sessions/` as JSON files.
...
- `acp-bridge <agent> sessions close [name]`
- `acp-bridge <agent> sessions new [--name <name>]` (for the replaced session)
...
`acp-bridge` stores the adapter pid in each session record to help with cleanup and diagnostics.

Lifecycle behavior:

- a queue owner `acp-bridge` process is elected per active session turn and accepts queued prompts over local IPC
