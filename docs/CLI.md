...
- Runs a user-authored workflow module step by step through the `acp-bridge/flows` runtime.
...
- text and quiet output print the local `acp-bridgeRecordId`; JSON output also includes
  `acp-bridgeSessionId` and, when the adapter exposes one, `agentSessionId`
...
- project: `<cwd>/.acp-bridgerc.json` (merged on top of global)
...
- no acp-bridge-specific event envelope,
