...
test("flowRunsBaseDir defaults under the acp-bridge home directory", () => {
  assert.equal(flowRunsBaseDir("/tmp/home"), path.join("/tmp/home", ".acp-bridge", "flows", "runs"));
});

test("FlowRunStore writes manifest, projections, flow snapshot, and trace events", async () => {
  const outputRoot = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-store-test-"));
  ...
  try {
    ...
    assert.equal(manifest.schema, "acp-bridge.flow-run-bundle.v1");
    assert.equal(manifest.runTitle, "Demo run title");
    assert.equal(manifest.paths.runProjection, "projections/run.json");
    assert.equal(flowSnapshot.schema, "acp-bridge.flow-definition-snapshot.v1");
...
test("FlowRunStore uses unique temp paths for concurrent live writes", async () => {
  const outputRoot = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-store-race-"));
...
test("FlowRunStore preserves bundled session event order across concurrent appends", async () => {
  const outputRoot = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-session-events-"));
