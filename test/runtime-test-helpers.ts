...
export function makeSessionRecord(
  ...
  const defaultAcpBridge = options.defaultAcpBridge ?? true;
  return {
    schema: "acp-bridge.session.v1",
    acp-bridgeRecordId: overrides.acp-bridgeRecordId,
    ...
    name: overrides.name ?? (defaultName ? overrides.acp-bridgeRecordId : undefined),
    ...
    acp-bridge: overrides.acp-bridge ?? (defaultAcpBridge ? {} : undefined),
  };
}
...
export function sessionFilePath(homeDir: string, acp-bridgeRecordId: string): string {
...
export async function writeSessionRecordFile(
  ...
  const filePath = sessionFilePath(homeDir, record.acp-bridgeRecordId);
...
export class InMemorySessionStore implements AcpSessionStore {
  ...
  constructor(initialRecords: SessionRecord[] = []) {
    for (const record of initialRecords) {
      this.records.set(record.acp-bridgeRecordId, structuredClone(record));
    }
  }
  ...
  async save(record: SessionRecord): Promise<void> {
    this.savedRecordIds.push(record.acp-bridgeRecordId);
    this.records.set(record.acp-bridgeRecordId, structuredClone(record));
  }
}
