...
test("runSessionSetModeDirect resumes a load-capable session and closes the client once", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const record = makeSessionRecord({
      acp-bridgeRecordId: "prompt-runner-resume",
    ...
    const persisted = await resolveSessionRecord(record.acp-bridgeRecordId);
...
test("runSessionSetConfigOptionDirect falls back to createSession and returns updated options", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const record = makeSessionRecord({
      acp-bridgeRecordId: "prompt-runner-config",
    ...
    assert.equal(persisted.protocolVersion, 1);
    assert.equal(persisted.closed, false);
    assert.deepEqual(persisted.acp-bridge?.desired_config_options, {
...
test("runSessionSetModelDirect updates current and desired model", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const record = makeSessionRecord({
      acp-bridgeRecordId: "prompt-runner-model",
    ...
    assert.equal(result.resumed, true);
    assert.equal(result.record.acp-bridge?.current_model_id, "gpt-5.4");
    assert.equal(result.record.acp-bridge?.session_options?.model, "gpt-5.4");

    const persisted = await resolveSessionRecord(record.acp-bridgeRecordId);
    assert.equal(persisted.acp-bridge?.current_model_id, "gpt-5.4");
    assert.equal(persisted.acp-bridge?.session_options?.model, "gpt-5.4");
  });
});

async function withTempHome(run: (homeDir: string) => Promise<void>): Promise<void> {
  await withTempHomeFixture("acp-bridge-prompt-runner-home-", run);
}
