...
test("formatPromptSessionBannerLine prints single-line prompt banner for matching cwd", () => {
  ...
  assert.equal(
    line,
    "[acp-bridge] session calm-forest (abc123) · /home/user/project · agent needs reconnect",
  );
});
...
test("formatPromptSessionBannerLine includes routed-from path when cwd differs", () => {
  ...
  assert.equal(
    line,
    "[acp-bridge] session calm-forest (abc123) · /home/user/project (routed from ./src/auth) · agent needs reconnect",
  );
});
...
test("CLI resolves unknown subcommand names as raw agent commands", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const session = makeSessionRecord({
      acp-bridgeRecordId: "custom-session",
...
test("sessions new --resume-session loads ACP session and stores resumed ids", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const payload = JSON.parse(result.stdout.trim()) as {
      action?: unknown;
      created?: unknown;
      acp-bridgeRecordId?: unknown;
      acp-bridgeSessionId?: unknown;
      agentSessionId?: unknown;
    };
    assert.equal(payload.action, "session_ensured");
    assert.equal(payload.created, true);
    assert.equal(payload.acp-bridgeRecordId, resumeSessionId);
    assert.equal(payload.acp-bridgeSessionId, resumeSessionId);
...
test("sessions new --resume-session surfaces not-found loadSession errors without fallback", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const sessionsDir = path.join(homeDir, ".acp-bridge", "sessions");
...
test("sessions ensure creates when missing and returns existing on subsequent calls", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(secondPayload.acp-bridgeRecordId, firstPayload.acp-bridgeRecordId);
  });
});
...
test("sessions ensure exits even when agent ignores SIGTERM", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(payload.action, "session_ensured");
    assert.equal(payload.created, true);
    assert.equal(typeof payload.acp-bridgeRecordId, "string");
    ...
    const storedRecord = JSON.parse(
      await fs.readFile(
        path.join(
          ...
          ".acp-bridge",
          "sessions",
          `${encodeURIComponent(payload.acp-bridgeRecordId as string)}.json`,
        ),
        "utf8",
      ),
    ) as SessionRecord;
...
test("sessions ensure resolves existing session by directory walk", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "parent-session",
    ...
    assert.equal(payload.acp-bridgeRecordId, "parent-session");
    assert.equal(payload.action, "session_ensured");
...
test("set-mode persists across load fallback and replays on fresh ACP sessions", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.notEqual(checkPlanPayload.acp-bridgeSessionId, setPlanPayload.acp-bridgeSessionId);
    ...
    assert.notEqual(checkAutoPayload.acp-bridgeSessionId, setAutoPayload.acp-bridgeSessionId);
    ...
    assert.equal(storedRecord.acp-bridge?.desired_mode_id, "auto");
  });
});
...
test("set-mode load fallback failure does not persist the fresh session id to disk", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(error.data?.acp-bridgeCode, "RUNTIME");
    ...
    assert.equal(storedRecord.acp-bridge?.desired_mode_id, "plan");
  });
});
...
test("--non-interactive-permissions validates supported values", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(error.data?.acp-bridgeCode, "USAGE");
...
test("prompt exits with NO_SESSION when no session exists (no auto-create)", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.match(
      result.stderr,
      new RegExp(
        `⚠ No acp-bridge session found \\(searched up to ${escapedCwd}\\)\\.\\nCreate one: acp-bridge codex sessions new\\n?`,
      ),
    );
  });
});
...
test("json format emits structured no-session error event", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(error.data?.acp-bridgeCode, "NO_SESSION");
    assert.match(error.message ?? "", /No acp-bridge session found/);
  });
});
...
test("set-mode exits with NO_SESSION when no session exists", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(result.code, 4);
    assert.match(result.stderr, /No acp-bridge session found/);
  });
});
...
test("cancel resolves named session when -s is before subcommand", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "named-cancel-session",
    ...
    assert.equal(payload.acp-bridgeRecordId, "named-cancel-session");
...
test("status resolves named session when -s is before subcommand", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "named-status-session",
    ...
    assert.equal(payload.acp-bridgeRecordId, "named-status-session");
...
test("status reports idle for resumable sessions without a live queue owner", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "idle-status-session",
...
test("set-mode resolves named session when -s is before subcommand", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const missingAgentCommand = "acp-bridge-test-missing-agent-binary";
    await fs.writeFile(
      path.join(homeDir, ".acp-bridge", "config.json"),
      `${JSON.stringify(
        {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "named-set-mode-session",
    ...
    assert.doesNotMatch(result.stderr, /No acp-bridge session found/);
...
test("set resolves named session when -s is before subcommand", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const missingAgentCommand = "acp-bridge-test-missing-agent-binary-2";
    await fs.writeFile(
      path.join(homeDir, ".acp-bridge", "config.json"),
      `${JSON.stringify(
        {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "named-set-config-session",
...
test("sessions history prints stored history entries", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "history-session",
...
test("sessions prune dry-run previews closed sessions without deleting", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "prune-dry-run",
...
test("sessions read prints full history by default and supports --tail", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "read-session",
...
test("config defaults are loaded from global and project config files", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await fs.writeFile(
      path.join(cwd, ".acp-bridgerc.json"),
      `${JSON.stringify(
        {
          agents: {
    ...
    await writeSessionRecord(homeDir, {
      acp-bridgeRecordId: "custom-config-session",
...
test("exec subcommand is blocked in json format when disableExec is true", async () => {
  await withTempHome(async (homeDir) => {
    ...
    assert.equal(payload.error?.data?.acp-bridgeCode, "EXEC_DISABLED");
  });
});
...
async function withTempHome(run: (homeDir: string) => Promise<void>): Promise<void> {
  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-cli-test-home-"));
  try {
    await run(tempHome);
  } finally {
...
async function writeSessionRecord(
  ...
  const file = path.join(sessionDir, `${encodeURIComponent(record.acp-bridgeRecordId)}.json`);
  await fs.writeFile(
    file,
...
function sessionFilePath(homeDir: string, acp-bridgeRecordId: string): string {
  return path.join(homeDir, ".acp-bridge", "sessions", `${encodeURIComponent(acp-bridgeRecordId)}.json`);
}
