...
async function withTempHome(run: (homeDir: string) => Promise<void>): Promise<void> {
  await withTempHomeFixture("acp-bridge-prune-test-", run);
}
...
test("pruneSessions returns empty result when no closed sessions exist", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "open-session",
...
test("pruneSessions deletes closed session files and removes them from the index", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "closed-session",
    ...
    assert.equal(result.pruned[0].acp-bridgeRecordId, "closed-session");
...
test("pruneSessions --dry-run does not delete files but returns correct count", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "dry-run-session",
...
test("pruneSessions --before uses closedAt before falling back to lastUsedAt", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "old-session",
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "recent-session",
    ...
    assert.equal(result.pruned.length, 1);
    assert.equal(result.pruned[0].acp-bridgeRecordId, "old-session");
...
test("pruneSessions --older-than prunes sessions beyond the day threshold", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "ancient-session",
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "fresh-session",
    ...
    assert.equal(result.pruned.length, 1);
    assert.equal(result.pruned[0].acp-bridgeRecordId, "ancient-session");
...
test("pruneSessions scoped to agentCommand only prunes that agent's sessions", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "agent-a-session",
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "agent-b-session",
    ...
    const result = await session.pruneSessions({ agentCommand: "agent-a" });
    assert.equal(result.pruned.length, 1);
    assert.equal(result.pruned[0].acp-bridgeRecordId, "agent-a-session");
...
test("pruneSessions --include-history deletes stream files", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await writeSessionRecord(
      homeDir,
      makeSessionRecord({
        acp-bridgeRecordId: "stream-session",
...
test("pruneSessions without agentCommand prunes all closed sessions across all agents", async () => {
  await withTempHome(async (homeDir) => {
    ...
    const prunedIds = result.pruned.map((r) => r.acp-bridgeRecordId).toSorted();
