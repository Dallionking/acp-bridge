...
test("parseReplayViewerCliArgs defaults to start and supports control flags", () => {
  assert.deepEqual(parseReplayViewerCliArgs([]), {
    ...
    host: "127.0.0.1",
    port: 4173,
    runsDir: path.join(os.homedir(), ".acp-bridge", "flows", "runs"),
    open: false,
  });
...
test("replay viewer status and stop helpers report and stop a running server", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-status-"));
  ...
  try {
    ...
    assert.deepEqual(health, {
      service: "acp-bridge-flow-replay-viewer",
      runsDir,
    });
...
test("replay viewer CLI status prints running details", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-status-cli-"));
  const viewerServer = await createReplayViewerServer({
    host: "127.0.0.1",
    port: 0,
    runsDir,
...
test("replay viewer start rejects reusing a server for a different runs dir", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-start-runs-a-"));
  const otherRunsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-start-runs-b-"));
  const viewerServer = await createReplayViewerServer({
    host: "127.0.0.1",
    port: 0,
    runsDir,
  ...
  try {
    await assert.rejects(
      ...
      /Viewer is already running .* not .*acp-bridge-replay-start-runs-b-/,
    );
  } finally {
    await viewerServer.close().catch(() => {});
    await fs.rm(runsDir, { recursive: true, force: true });
...
test("replay viewer blocks file reads that escape the runs directory via runId", async () => {
  const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-traversal-"));
  const runsDir = path.join(fakeHome, ".acp-bridge", "flows", "runs");
  const sessionsDir = path.join(fakeHome, ".acp-bridge", "sessions");
