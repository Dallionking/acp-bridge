...
test("queue path helpers derive stable lock and socket paths", () => {
  ...
  assert.equal(queueBaseDir(homeDir), "/tmp/example-home/.acp-bridge/queues");
  ...
  if (process.platform === "win32") {
    ...
    assert.equal(queueSocketPath("session-id", homeDir), `\\\\.\\pipe\\acp-bridge-${key}`);
    return;
  }

  assert.equal(queueSocketBaseDir(homeDir)?.startsWith("/tmp/acp-bridge-"), true);
...
test("queueSocketPath stays short on unix even for long home paths", () => {
  ...
  assert(socketPath.startsWith("/tmp/acp-bridge-"));
