...
test("replay viewer streams live sidebar and run patches over websocket", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-live-"));
  ...
  await writeRunBundle(runsDir, {
    runId,
    flowName: "pr-triage",
    runTitle: "PR-triage-acp-bridge-155",
    startedAt,
  ...
  try {
    ...
    socket.send(JSON.stringify({ type: "hello", protocol: "acp-bridge.replay.v1" }));
    ...
    assert.equal(listViewerRuns(runsSnapshot.state)[0]?.runTitle, "PR-triage-acp-bridge-155");
...
test("computeResourceDelta produces a stable patch when recent runs reorder", () => {
  const firstRun = {
    ...
    path: "/tmp/acp-bridge-live-run-a",
  };
  const secondRun = {
    ...
    path: "/tmp/acp-bridge-live-run-b",
  };
...
test("replay viewer refreshes runs snapshots after idle periods", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-live-runs-refresh-"));
  ...
  try {
    ...
    firstSocket.send(JSON.stringify({ type: "hello", protocol: "acp-bridge.replay.v1" }));
    ...
    await writeRunBundle(runsDir, {
      runId: secondRunId,
      flowName: "pr-triage",
      runTitle: "PR-triage-acp-bridge-156",
    ...
    secondSocket.send(JSON.stringify({ type: "hello", protocol: "acp-bridge.replay.v1" }));
...
test("replay viewer streams selected-run ACP text as JSON Patch+ append updates", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-replay-live-session-"));
...
async function writeRunBundle(
  ...
  const manifest: FlowRunManifest = {
    schema: "acp-bridge.flow-run-bundle.v1",
    runId: options.runId,
    flowName: options.flowName,
    runTitle: options.runTitle,
    startedAt: options.startedAt,
    status: options.liveStatus,
    traceSchema: "acp-bridge.flow-trace-event.v1",
    paths: {
      flow: "flow.json",
      trace: "trace.ndjson",
...
function makeFlow(): FlowDefinitionSnapshot {
  return {
    schema: "acp-bridge.flow-definition-snapshot.v1",
    name: "pr-triage",
...
async function writeLiveSessionRunBundle(
  ...
  const binding: FlowSessionBinding = {
    ...
    agentCommand: "codex",
    cwd: "/tmp/replay-live-sync",
    acp-bridgeRecordId: "session-record",
