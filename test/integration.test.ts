...
test("integration: exec echo baseline", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
...
test("integration: built-in cursor agent resolves to cursor-agent acp", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-cursor-"));
  ...
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const payload = JSON.parse(result.stdout.trim()) as {
        ...
        sessionBindings?: Record<string, { acp-bridgeRecordId: string }>;
      };
...
test("integration: flow run finalizes interrupted bundles on SIGHUP", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const outputRoot = path.join(homeDir, ".acp-bridge", "flows", "runs");
...
test("integration: flow run fails fast when a flow requires an explicit approve-all grant", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-permission-cwd-"));
    const flowDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-permission-"));
    ...
    try {
      await fs.writeFile(
        flowPath,
        [
          'import { compute, defineFlow } from "acp-bridge/flows";',
          "",
...
test("integration: flow run requires defineFlow before permission gating", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      assert.match(
        result.stderr,
        /Flow module must export default defineFlow\(\{\.\.\.\}\) from "acp-bridge\/flows"/,
      );
...
test("integration: flow run preserves approve-all through persistent ACP writes", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-write-cwd-"));
    const flowDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-write-"));
    ...
    try {
      await fs.writeFile(
        flowPath,
        [
          'import { acp, defineFlow } from "acp-bridge/flows";',
          "",
          "export default defineFlow({",
...
test('integration: flow run resolves "acp-bridge/flows" imports for external flow files', async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const flowDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-import-"));
...
test("integration: flow run supports staged defineFlow assembly in external modules", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const flowDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-flow-staged-"));
...
test("integration: built-in droid agent resolves to droid exec --output-format acp", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-droid-"));
...
test("integration: built-in iflow agent resolves to iflow --experimental-acp", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-iflow-"));
...
test("integration: built-in qoder agent resolves to qodercli --acp", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-qoder-"));
    ...
    try {
      ...
      try {
        ...
        const result = await runSessionSetModeDirect({
          sessionRecordId: record.acp-bridgeRecordId,
          modeId: "plan",
          timeoutMs: 10_000,
        });
        assert.equal(result.record.acp-bridgeRecordId, record.acp-bridgeRecordId);
      } finally {
...
test("integration: perf metrics capture writes ndjson records for CLI runs", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      const result = await runCli([...baseExecArgs(cwd), "echo hello"], homeDir, {
        env: {
          ACP_BRIDGE_PERF_METRICS_FILE: metricsPath,
        },
      });
      assert.equal(result.code, 0, result.stderr);
...
test("integration: perf report tolerates malformed lines and keeps role and gauge summaries", async () => {
  const metricsPath = path.join(os.tmpdir(), `acp-bridge-perf-report-${Date.now()}.ndjson`);
...
test("integration: perf metrics capture preserves SIGTERM termination semantics", async () => {
  const metricsPath = path.join(os.tmpdir(), `acp-bridge-perf-signal-${Date.now()}.ndjson`);
...
test("integration: configured mcpServers are sent to session/new and session/load", async () => {
  await withTempHome(async (homeDir) => {
    ...
    await fs.mkdir(path.join(homeDir, ".acp-bridge"), { recursive: true });
    await fs.writeFile(
      path.join(homeDir, ".acp-bridge", "config.json"),
      `${JSON.stringify(
        {
          mcpServers: [
            {
    ...
    try {
      ...
      sessionId = createdPayload.acp-bridgeRecordId;
      ...
      const payloads = result.stdout
        ...
        .map(
          (line) =>
            JSON.parse(line) as {
              jsonrpc?: string;
              error?: { code?: number; data?: { acp-bridgeCode?: string } };
            },
        );
      assert(payloads.length > 0, "expected at least one JSON payload");
      const timeoutError = payloads.find(
        (payload) => payload.jsonrpc === "2.0" && payload.error?.data?.acp-bridgeCode === "TIMEOUT",
      );
...
test("integration: gemini ACP startup timeout is surfaced as actionable error for gemini.cmd too", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-gemini-"));
    const fakeGeminiPath = path.join(fakeBinDir, "gemini.cmd");
    const previousTimeout = process.env.ACP_BRIDGE_GEMINI_ACP_STARTUP_TIMEOUT_MS;
    ...
    try {
      ...
      process.env.ACP_BRIDGE_GEMINI_ACP_STARTUP_TIMEOUT_MS = "100";
      ...
      const payloads = result.stdout
        ...
        .map(
          (line) =>
            JSON.parse(line) as {
              error?: { message?: string; data?: { acp-bridgeCode?: string; detailCode?: string } };
            },
        );
      const timeoutError = payloads.find(
      ...
      assert.equal(timeoutError.error?.data?.acp-bridgeCode, "TIMEOUT");
      ...
      if (previousTimeout == null) {
        delete process.env.ACP_BRIDGE_GEMINI_ACP_STARTUP_TIMEOUT_MS;
      } else {
        process.env.ACP_BRIDGE_GEMINI_ACP_STARTUP_TIMEOUT_MS = previousTimeout;
      }
      await fs.rm(fakeBinDir, { recursive: true, force: true });
      await fs.rm(cwd, { recursive: true, force: true });
    }
...
test("integration: copilot ACP unsupported binary is surfaced as actionable error", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-copilot-"));
    ...
    try {
      ...
      assert.equal(unsupportedError.error?.data?.acp-bridgeCode, "RUNTIME");
...
test("integration: claude ACP session creation timeout is surfaced as actionable error", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const fakeBinDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-fake-claude-acp-"));
    const fakeClaudeAcpPath = path.join(fakeBinDir, "claude-agent-acp");
    const previousTimeout = process.env.ACP_BRIDGE_CLAUDE_ACP_SESSION_CREATE_TIMEOUT_MS;
    ...
    try {
      ...
      process.env.ACP_BRIDGE_CLAUDE_ACP_SESSION_CREATE_TIMEOUT_MS = "100";
      ...
      assert.match(timeoutError.error?.message ?? "", /acp-bridge claude exec/i);
    } finally {
      if (previousTimeout == null) {
        delete process.env.ACP_BRIDGE_CLAUDE_ACP_SESSION_CREATE_TIMEOUT_MS;
      } else {
        process.env.ACP_BRIDGE_CLAUDE_ACP_SESSION_CREATE_TIMEOUT_MS = previousTimeout;
      }
      await fs.rm(fakeBinDir, { recursive: true, force: true });
      await fs.rm(cwd, { recursive: true, force: true });
    }
...
test("integration: fs/read_text_file through mock agent", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const readPath = path.join(cwd, "acp-bridge-test-read.txt");
...
test("integration: --suppress-reads hides read file body in text format", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const readPath = path.join(cwd, "acp-bridge-test-read-tools.txt");
    await fs.writeFile(readPath, "mock read content", "utf8");
...
test("integration: --suppress-reads hides read file body in json format", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const readPath = path.join(cwd, "acp-bridge-test-read-json.txt");
    await fs.writeFile(readPath, "mock read content", "utf8");
...
test("integration: fs/write_text_file through mock agent", async () => {
  await withTempHome(async (homeDir) => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-cwd-"));
    const writePath = path.join(cwd, "acp-bridge-test-write.txt");
...
test("integration: prompt reuses warm queue owner and agent pid across turns", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const sessionId = createdEvent.acp-bridgeRecordId;
      assert.equal(typeof sessionId, "string");
...
test("integration: config agent command with flags is split correctly and stores protocol version", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const sessionId = createdPayload.acp-bridgeRecordId;
      assert.equal(typeof sessionId, "string");
...
test("integration: prompt recovers when loadSession fails on empty session without emitting load error", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const originalSessionId = createdEvent.acp-bridgeRecordId;
...
test("integration: prompt exits after done while detached owner stays warm", async () => {
  await withTempHome(async (homeDir) => {
    ...
    try {
      ...
      const sessionId =
        createdPayload.acp-bridgeRecordId ??
        createdPayload.acp-bridge_record_id ??
...
async function withTempHome(run: (homeDir: string) => Promise<void>): Promise<void> {
  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-integration-home-"));
  try {
    await run(tempHome);
  } finally {
...
async function runCliWithEntry(
  ...
  return await new Promise<CliRunResult>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error(`CLI timed out after ${timeoutMs}ms: acp-bridge ${args.join(" ")}`));
    }, timeoutMs);
...
function queueOwnerLockPath(homeDir: string, sessionId: string): string {
  ...
  return path.join(homeDir, ".acp-bridge", "queues", `${queueKey}.lock`);
}
