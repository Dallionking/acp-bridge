...
test("listRunBundles returns newest valid bundles first", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-run-list-"));

  try {
    await writeRunBundle(runsDir, {
      runId: "2026-03-27T060000000Z-example-a",
      flowName: "flow-a",
      runTitle: "PR-triage-acp-bridge-171",
      status: "completed",
    ...
    assert.equal(runs[1]?.runTitle, "PR-triage-acp-bridge-171");
  } finally {
...
test("listRunBundles prefers live status over stale run projections", async () => {
  const runsDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-run-list-live-"));
...
test("resolveRunBundleFilePath rejects traversal outside a run bundle", () => {
  const runsDir = path.join(os.tmpdir(), "acp-bridge-run-list");
