...
test("conversation model captures prompt, chunks, tool calls, and metadata", () => {
  ...
  acp-bridgeState = recordClientOperation(
    conversation,
    acp-bridgeState,
    {
  ...
  assert.equal(acp-bridgeState?.current_mode_id, "code");
  assert.deepEqual(acp-bridgeState?.available_commands, ["create_plan"]);
});
...
test("cloneSessionAcpBridgeState preserves desired mode id", () => {
  const cloned = cloneSessionAcpBridgeState({
