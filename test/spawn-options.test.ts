...
test("buildAgentSpawnOptions hides Windows console windows and preserves auth env", () => {
  const options = buildAgentSpawnOptions("/tmp/acp-bridge-agent", {
    ACP_BRIDGE_AUTH_TOKEN: "secret-token",
  });

  assert.equal(options.cwd, "/tmp/acp-bridge-agent");
  assert.deepEqual(options.stdio, ["pipe", "pipe", "pipe"]);
  assert.equal(options.windowsHide, true);
  assert.equal(options.env.ACP_BRIDGE_AUTH_TOKEN, "secret-token");
});

test("buildAgentSpawnOptions promotes explicit ACP_BRIDGE auth env vars into agent auth env", () => {
  const previousPrefixed = process.env.ACP_BRIDGE_AUTH_OPENAI_API_KEY;
  const previousNormalized = process.env.OPENAI_API_KEY;

  process.env.ACP_BRIDGE_AUTH_OPENAI_API_KEY = "sk-explicit";
  delete process.env.OPENAI_API_KEY;

  try {
    const options = buildAgentSpawnOptions("/tmp/acp-bridge-agent", undefined);
    assert.equal(options.env.ACP_BRIDGE_AUTH_OPENAI_API_KEY, "sk-explicit");
    assert.equal(options.env.OPENAI_API_KEY, "sk-explicit");
  } finally {
    if (previousPrefixed == null) {
      delete process.env.ACP_BRIDGE_AUTH_OPENAI_API_KEY;
    } else {
      process.env.ACP_BRIDGE_AUTH_OPENAI_API_KEY = previousPrefixed;
    }
...
test("buildTerminalSpawnOptions hides Windows console windows and maps env entries", () => {
  const options = buildTerminalSpawnOptions("node", "/tmp/acp-bridge-terminal", [
    { name: "TMUX", value: "/tmp/tmux-1000/default,123,0" },
    { name: "TERM", value: "screen-256color" },
  ]);

  assert.equal(options.cwd, "/tmp/acp-bridge-terminal");
...
test("buildQueueOwnerSpawnOptions hides Windows console windows and passes payload", () => {
  ...
  assert.equal(options.windowsHide, true);
  assert.equal(options.env.ACP_BRIDGE_QUEUE_OWNER_PAYLOAD, '{"sessionId":"queue-session"}');
});
...
test("buildSpawnCommandOptions enables shell for PATH-resolved .cmd wrappers on Windows", async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "acp-bridge-windows-spawn-"));
  const env = {
    PATH: tempDir,
