...
import { encodeAcpBridgeRuntimeHandleState } from "../src/runtime/public/handle-state.js";
import {
  asOptionalBoolean,
  asOptionalString,
  asString,
...
test("runtime handle state codecs preserve valid payloads and reject invalid ones", () => {
  ...
  assert.equal(decodeAcpBridgeRuntimeHandleState("acp-bridge:v2:bad-json"), null);
  assert.equal(
    decodeAcpBridgeRuntimeHandleState(
      "acp-bridge:v2:" +
        Buffer.from(
...
test("runtime lifecycle helpers update records from runtime snapshots and conversations", () => {
  const record = makeSessionRecord({
    acp-bridgeRecordId: "lifecycle-record",
    acpSessionId: "sid-1",
