...
async function withTempDir(run: (dir: string) => Promise<void>): Promise<void> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "acp-bridge-queue-owner-path-"));
  try {
    await run(dir);
  } finally {
...
describe("resolveQueueOwnerSpawnArgs", () => {
  it("prefers ACP_BRIDGE_QUEUE_OWNER_ARGS when provided", () => {
    const previous = process.env.ACP_BRIDGE_QUEUE_OWNER_ARGS;
    process.env.ACP_BRIDGE_QUEUE_OWNER_ARGS = JSON.stringify([
      "--import",
      "tsx",
      "src/cli.ts",
      "__queue-owner",
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ACP_BRIDGE_QUEUE_OWNER_ARGS = previous;
      }
    }
  });
  ...
  it("returns <real cli path> and __queue-owner", async () => {
    await withTempDir(async (dir) => {
      ...
      const cliLink = path.join(dir, "acp-bridge-link.js");
  ...
  it("throws when argv lacks an entry path", () => {
    assert.throws(() => resolveQueueOwnerSpawnArgs(["node"]), {
      message: "acp-bridge self-spawn failed: missing CLI entry path",
    });
  });
});
