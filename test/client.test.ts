...
test("buildAgentSpawnOptions normalizes auth env keys and preserves existing values", () => {
  withEnv(
    {
      ACP_BRIDGE_AUTH_API_TOKEN: "existing-prefixed",
    ...
    () => {
      ...
      assert.equal(options.env.ACP_BRIDGE_AUTH_API_TOKEN, "existing-prefixed");
      assert.equal(options.env.API_TOKEN, "existing-normalized");
      assert.equal(options.env.EXPLICIT_KEY, "explicit");
      assert.equal(options.env.ACP_BRIDGE_AUTH_EXPLICIT_KEY, "explicit");
      assert.equal(options.env["bad=key"], undefined);
      assert.equal(options.env.ACP_BRIDGE_AUTH_BAD_KEY, "ignored-for-raw-key");
...
test("AcpClient prefers env auth credentials over config credentials", async () => {
  await withEnv(
    {
      ACP_BRIDGE_AUTH_API_TOKEN: "from-env",
    },
    async () => {
  ...
  await withEnv(
    {
      ...
      ACP_BRIDGE_AUTH_OPENAI_API_KEY: undefined,
    },
    async () => {
      const client = makeClient();
      const internals = asInternals(client);
...
test("AcpClient createSession forwards claudeCode options in _meta", async () => {
  ...
  const result = await client.createSession("/tmp/acp-bridge-client-meta");
  assert.equal(result.sessionId, "session-123");
  assert.deepEqual(capturedParams, {
    cwd: "/tmp/acp-bridge-client-meta",
    mcpServers: [],
    _meta: {
      claudeCode: {
        options: {
...
test("AcpClient createSession forwards systemPrompt string in _meta", async () => {
  ...
  await client.createSession("/tmp/acp-bridge-client-system-prompt");
  assert.deepEqual(capturedParams, {
    cwd: "/tmp/acp-bridge-client-system-prompt",
    mcpServers: [],
    _meta: {
      systemPrompt: "you are an obsidian assistant",
    },
...
test("AcpClient createSession forwards systemPrompt append in _meta alongside claudeCode options", async () => {
  ...
  await client.createSession("/tmp/acp-bridge-client-system-prompt-append");
  assert.deepEqual(capturedParams, {
    cwd: "/tmp/acp-bridge-client-system-prompt-append",
    mcpServers: [],
    _meta: {
      claudeCode: {
        options: {
...
test("AcpClient createSession forwards codex model metadata without setting it explicitly", async () => {
  ...
  const result = await client.createSession("/tmp/acp-bridge-client-codex-model");
  assert.equal(result.sessionId, "session-456");
  assert.deepEqual(capturedNewSessionParams, {
    cwd: "/tmp/acp-bridge-client-codex-model",
    mcpServers: [],
    _meta: {
      claudeCode: {
        options: {
