...
test("json formatter emits ACP JSON-RPC error response from onError", () => {
  ...
  assert.equal(parsed.error?.data?.acp-bridgeCode, "RUNTIME");
...
test("text formatter prints auth remediation hints", () => {
  ...
  assert.match(output, /hint: run `acp-bridge config show`/);
...
test("quiet formatter emits final usage and cost metadata to stderr", () => {
  ...
  assert.equal(
    stderr.toString(),
    "[acp-bridge] tokens: input=17030 output=4 cache_read=12 cache_write=3 total=17049\n[acp-bridge] cost: 0.051276 USD\n",
  );
});
